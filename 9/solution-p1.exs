defmodule HeldKarpStrategy do
  ...
  def shortest(dists) do
    0..Enum.count(dists)-1
        |> Enum.map(&shortest_from_node(&1,dists))
        |> Enum.reduce({@infinite, []}, &reduce_to_minimum/2)
  end

  defp shortest_from_node(start_node, dists) do
    ...
    {min_cost, end_node} = final_nodes |> Enum.reduce({@infinite, {}}, fn (key, acc) -> best_node(key, acc, costs) end)
  ...
  defp calculate_costs(start_node, nodes_to_hit, dists) do
    create_subsets(Enum.count(dists)-1, nodes_to_hit)
        ...
        |> Enum.reduce(%{}, fn (key, acc) -> calculate_minimum_costs(key, acc, start_node, dists) end)
  end
  ...
  defp calculate_minimum_costs({i, subset}, costs_map, start_node, dists) do
  ...
  defp best_node(key, {cur_min, cur_key}, costs) do
...
IO.inspect HeldKarpStrategy.shortest(distances)